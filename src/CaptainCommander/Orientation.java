...
//		multiplyByMatrix(reversetotal);
		...
		cen = new Double[]{relcen[0]+eye[0],relcen[1]+eye[1],relcen[2]+eye[2]};
		up = new Double[]{relup[0]+eye[0],relup[1]+eye[1],relup[2]+eye[2]};
		if(usemisc){ //And reapply to the stored class variables
			Double [][] relmisc2 = new Double[misc.length][];
			for(int i = 0; i < misc.length; i++){
				relmisc[i] = normalize(relmisc[i]);
				relmisc2[i] = new Double[]{relmisc[i][0]+eye[0],relmisc[i][1]+eye[1],relmisc[i][2]+eye[2]};
			}
			misc = relmisc2;
		}
	}
	...
	protected void reevaluate(double roll, double pitch, boolean disable){
		double tx = Math.cos(roll)*up[0] - Math.sin(roll)*temp[0];
		double ty = Math.cos(roll)*up[1] - Math.sin(roll)*temp[1];
		double tz = Math.cos(roll)*up[2] - Math.sin(roll)*temp[2];
		temp[0] = Math.sin(roll)*up[0] + Math.cos(roll)*temp[0];
		temp[1] = Math.sin(roll)*up[1] + Math.cos(roll)*temp[1];
		temp[2] = Math.sin(roll)*up[2] + Math.cos(roll)*temp[2];
		up[0] = tx;
		up[1] = ty;
		up[2] = tz;
		tx = Math.cos(pitch)*up[0] + Math.sin(pitch)*cen[0];
		ty = Math.cos(pitch)*up[1] + Math.sin(pitch)*cen[1];
		tz = Math.cos(pitch)*up[2] + Math.sin(pitch)*cen[2];
		cen[0] = -Math.sin(pitch)*up[0] + Math.cos(pitch)*cen[0];
		cen[1] = -Math.sin(pitch)*up[1] + Math.cos(pitch)*cen[1];
		cen[2] = -Math.sin(pitch)*up[2] + Math.cos(pitch)*cen[2];
		up[0] = tx;
		up[1] = ty;
		up[2] = tz;
		//        eye[0][0] -= cen[0][0];
